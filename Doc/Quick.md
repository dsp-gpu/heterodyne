...
| Задача | Класс |
|--------|-------|
| Получить f_beat + дальность + SNR | `HeterodyneDechirp` (C++) или `dsp_heterodyne.HeterodyneDechirp` (Python) |
| Встроить дечирп в свой пайплайн (без FFT) | `dsp_heterodyne.HeterodyneROCm` (Python, ROCm-only) |
...
- **core** → `IBackend*`, `GPUProfiler`, `ConsoleOutput`, `KernelCacheService`
...
```python
import dsp_heterodyne, numpy as np
