...
**Каталог**: `heterodyne/`
**Зависимости**: core (`IBackend*`), signal_generators (LfmConjugateGenerator), fft_maxima (SpectrumMaximaFinder), OpenCL / ROCm (HIP)
...
**Python классы**:
- `dsp_heterodyne.HeterodyneDechirp` — полный пайплайн (OpenCL или ROCm), возвращает f_beat + range
- `dsp_heterodyne.HeterodyneROCm` — низкоуровневый ROCm-only: `dechirp()` + `correct()` без FFT
...
| [Doc_Addition/Info_ROCm_HIP_Optimization_Guide.md](../../../Doc_Addition/Info_ROCm_HIP_Optimization_Guide.md) | Оптимизация HIP/ROCm ядер — теория и проверенные паттерны DSP-GPU |
